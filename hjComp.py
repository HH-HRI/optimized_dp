...
# mode: arc, rect, None 
def ShapeMoveAvoid(grid, params, tau, static):
  #initial state
  x0,y0,v0 = params['obst']['state']
  ...
  for timestep in range(len(tau)):
    ...
    if mode =='arc':
      ...
      '''
      data_C_1 = ShapeRectangle(grid, [x_back, -np.inf, y_back_R, -np.inf, -np.inf, -np.inf], [x_front, np.inf, y_front_L, np.inf, np.inf, np.inf])
      data_C_2 = ShapeRectangle(grid, [-np.inf, x_back, -np.inf, y_back_R, -np.inf, -np.inf], [np.inf, x_front, np.inf, y_front_L, np.inf, np.inf])
      data_C = Union(data_C_1, data_C_2)
      data = Union(data, data_C)
...
def HJComp(params, idx):
  ...
  extraArgs['obstacles'] = ShapeMoveAvoid(g, params, tau, HJ_staticAvoid)
