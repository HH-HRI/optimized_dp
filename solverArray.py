...
def HJSolver(dynamics_obj, grid, init_value, tau, compMethod, plot_option, extraArgs, idx):
    ...
    if grid.dims == 5:
        # saving optimal controls
        Ctrl = hcl.asarray(np.zeros(tuple(np.insert(grid.pts_each_dim,0,2))))
        solve_pde = graph_5D(dynamics_obj, grid, compMethod)
        init_ctrl = getControls_5D(dynamics_obj, grid)
    if grid.dims == 6:
        # saving optimal controls
    ...
    if grid.dims == 5:
        init_ctrl(Ctrl, V_0, list_x1, list_x2, list_x3, list_x4, list_x5)
        print('empty array')
        valfuns = np.zeros(np.insert(tuple(grid.pts_each_dim), 5, len(tau)))
        ctrls = np.zeros(np.insert(tuple(grid.pts_each_dim), [5,0], [len(tau),2]))
        print('initial array value')
        valfuns[:,:,:,:,:,-1] = V_0.asnumpy()
        print('ctrls')
        ctrls[:,:,:,:,:,:,-1] = Ctrl.asnumpy()
    ...
    if grid.dims == 6:
        ...
        ctrls = np.zeros(np.insert(tuple(grid.pts_each_dim), [6,0], [len(tau),2]))
        print('initial array value')
        valfuns[:,:,:,:,:,:,-1] = V_0.asnumpy()
        print('ctrls')
        ctrls[:,:,:,:,:,:,:,-1] = Ctrl.asnumpy()
    ...
    for i in range (1, len(tau)):
        ...
        if grid.dims == 5:
            init_ctrl(Ctrl, V_1, list_x1, list_x2, list_x3, list_x4, list_x5)
            valfuns[:,:,:,:,:, -1-i] = V_1.asnumpy()
            ctrls[:,:,:,:,:,:, -1-i] = Ctrl.asnumpy()
        if grid.dims == 6:
    ...
    filename2 = "controls" + str(idx) + '.npy'
    np.save(filename, valfuns)
    #np.save("new_center_final.npy", V_1.asnumpy())
    np.save(filename2, ctrls)
    #np.save(filename2, ctrls.astype(bool))


    #print(ctrls.astype(bool))
